...
class RssSubscribe(_PluginBase):
    ...
    def get_form(self) -> Tuple[List[dict], Dict[str, Any]]:
        ...
        """
        category_priority_rows = self.__build_category_priority_rows()
                            ...
                            'content': [
                                {
                                    ...
                                    'props': {
                                        'type': 'info',
                                        'variant': 'tonal',
                                        'text': '以下选项来自[设定-规则]中设置了二级分类的规则组，按分类分别设置即时推送级别；未单独设置时回退到全局[即时推送优先级]。'
                                    }
                                }
                            ]
                        }
        ...
        for group_name in self.__extract_subscribe_categories():
    ...
    def __extract_subscribe_categories(self) -> Dict[str, int]:
        """从[设定-规则]中提取所有已配置二级分类的规则组，返回 {分类名: 优先级档数}。"""
        all_groups: list = self.systemconfig.get(
            SystemConfigKey.UserFilterRuleGroups) or []
        ...
        for group in all_groups:
            ...
            tier_count = len([s for s in rule_string.split(">") if s.strip()]) if rule_string else 0
            if category not in categories or tier_count > categories[category]:
                categories[category] = tier_count
        return categories

    def __build_category_priority_rows(self) -> List[dict]:
        category_tiers = self.__extract_subscribe_categories()
        rows = []

        group_names = list(category_tiers.keys())
        for index in range(0, len(group_names), 3):
            cols = []
            for group_name in group_names[index:index + 3]:
                tier_count = category_tiers.get(group_name, 0)
                priority_items = [{'title': '禁用', 'value': 0}]
                for i in range(1, tier_count + 1):
                    if i == 1:
                        priority_items.append({'title': f'优先级{i}（仅最高档）', 'value': i})
                    else:
                        priority_items.append({'title': f'优先级{i}（前{i}档）', 'value': i})
                cols.append({
                    'component': 'VCol',
                    'props': {
                        'cols': 12,
                    ...
                    'content': [
                        {
                            ...
                            'props': {
                                ...
                                'hint': f'二级分类：{group_name}（共{tier_count}档）',
    ...
    def __extract_category_priority_from_config(self, config: dict) -> Dict[str, int]:
        ...
        category_tiers = self.__extract_subscribe_categories()
        result = {}
        for group_name, tier_count in category_tiers.items():
            model_name = self.__get_category_priority_model_name(group_name)
            if model_name not in config:
                continue
            try:
                result[group_name] = max(0, min(tier_count, int(config.get(model_name) or 0)))
            except Exception:
                result[group_name] = 0
        return result
